...
Карточка сообщения содержит информацию о времени отправки сообщения (заметим, что если задержать курсор мышки на поле со временем отправки, то подсказка покажет полное время отправки в формате GMT), а также несколько полезных иконок, позволяющих «схлопнуть» сообщение, показать текст сообщения в режимах «Markdown включён», «Markdown выключён», «Код включён», и, только для администраторов, «спрятать» сообщение от не-администраторов.
