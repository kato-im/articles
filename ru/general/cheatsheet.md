...
Понять, активен ли определенный пользователь Kato, можно с помощью таких иконок статусов:
...
Для того, чтобы сохранить текущее число открытых комнат при открытии новой, необходимо нажать клавишу _Alt_ в двух перечисленных случаях:

- при клике на комнате в ростере;
...
Упомянутый таким образом пользователь получить оповещение об этом — в нужной комнате ему будет показана красная иконка в форме пилюли, сигнализирующая о наличии в этой комнате упоминания. Кроме того, на адрес электронной почты пользователя будет отправлено соответствующее письмо. Также появятся обычное всплывающее оповещение в десктоп-версии сервиса и обычное push-уведомление в мобильном клиенте. 

********************************************************************************
## <a href="#group-mentions" name="group-mentions">Упоминания групп</a>

Для привлечения внимания целой группы пользователей Kato, следует упомянуть комнату, на которую они **подписаны**, то есть уровень шума для которой установлен на **Follow**. В таком случае, все пользователи, которые подписаны на нужную комнату, получат оповещение.
...
Адресат подобного Email-сообщения может ответить на него — в таком случае ответ будет опубликован в той же комнате Kato, где было размещено изначальное непрочитанное сообщение.
...
Большинство команд клавиатурного контроля набираются в специальном **командном режиме** и состоят из одного символа (по аналогии с Unix-редактором [vi/vim](http://en.wikipedia.org/wiki/Vi)). Для переключения в командный режим следует нажать клавишу _escape_. Нажатие клавиши _?_ выводит список всех доступных команд (он также доступен по ссылке `https://app.kato.im/#/help/keyboard-control`).
